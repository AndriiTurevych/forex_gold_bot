...
def test_full_causal_pipeline_freezes_manifest_hash():
    d1, d2, snapshots, prior_dates, responses = two_day_inputs()
    requests = build_liquidity_request_plan(
        snapshots, dates=[d1, d2], prior_session_date_by_as_of=prior_dates
    )
    calendar = build_calendar_from_liquidity_responses(
        snapshots, requests=requests, responses_by_as_of=responses
    )
    ...
    pages = {
        "GCQ5": [{
            "request_id": "p1",
            "results": [
                bar("GCQ5", ns(2025, 6, 2, 12), 3300.0),
                bar("GCQ5", ns(2025, 6, 3, 12), 3310.0),
            ],
        }]
    }
    ...
    assert result1.manifest.rows == 2
...
def test_manifest_hash_changes_when_authoritative_bar_changes():
    ...
    base = {"GCQ5": [{"results": [bar("GCQ5", ns(2025, 6, 2), 3300.0), bar("GCQ5", ns(2025, 6, 3), 3310.0)]}]}
    changed = {"GCQ5": [{"results": [bar("GCQ5", ns(2025, 6, 2), 3300.0), bar("GCQ5", ns(2025, 6, 3), 3311.0)]}]}
...
def test_request_plan_rejects_future_or_same_day_liquidity():
    ...
    with pytest.raises(ValueError, match="strictly before"):
        build_liquidity_request_plan(
            snapshots, dates=[d1], prior_session_date_by_as_of={d1: d1}
        )
...
def test_evidence_assembly_rejects_incomplete_pagination():
    ...
    requests = build_liquidity_request_plan(snapshots, dates=[d1, d2], prior_session_date_by_as_of=prior_dates)
    calendar = build_calendar_from_liquidity_responses(snapshots, requests=requests, responses_by_as_of=responses)
    pages = {"GCQ5": [{"next_url": "still-more", "results": [bar("GCQ5", ns(2025, 6, 2), 3300.0)]}]}
...
def test_evidence_assembly_marks_first_bar_after_roll():
    d1, d2, snapshots, prior_dates, responses = two_day_inputs()
    responses[d1]["GCM5"][0]["volume"] = 200000
    responses[d1]["GCQ5"][0]["volume"] = 100000
    requests = build_liquidity_request_plan(snapshots, dates=[d1, d2], prior_session_date_by_as_of=prior_dates)
    calendar = build_calendar_from_liquidity_responses(snapshots, requests=requests, responses_by_as_of=responses)
    pages = {
        "GCM5": [{"results": [bar("GCM5", ns(2025, 6, 2), 3280.0)]}],
        "GCQ5": [{"results": [bar("GCQ5", ns(2025, 6, 3), 3310.0)]}],
    }
