...
def test_full_sequence_emits_replay_setup_on_first_later_retest():
    ...
    setups = assemble_replay_setups(
        ...
        htf_permission={4: DirectionalPermission(True, False)}, horizon_bars=60,
    )
    assert len(setups) == 1
    s = setups[0]
...
def test_directional_permission_blocks_wrong_side():
    bars = [_bar(0), _bar(1, low=89.0), _bar(2, close=106.0, low=100.0, high=107.0),
            _bar(3, close=103.0, low=102.5, high=104.0), _bar(4, close=101.0, low=100.5, high=102.5)]
    setups = assemble_replay_setups(
        bars=bars, events=_long_events(bars), context=[_ctx(i) for i in range(5)],
        htf_permission={4: DirectionalPermission(False, True)}, horizon_bars=60,
    )
    assert len(setups) == 1
    assert setups[0].htf_location_ok is False
...
def test_fvg_same_bar_is_not_treated_as_retest():
    ...
    setups = assemble_replay_setups(
        ...
        htf_permission={3: DirectionalPermission(True, False)}, horizon_bars=60,
    )
    assert setups == ()
...
def test_out_of_range_event_and_context_fail_closed():
    bars = [_bar(0)]
    with pytest.raises(ValueError, match="context index outside"):
        assemble_replay_setups(bars=bars, events=[], context=[_ctx(2)], htf_permission={}, horizon_bars=60)
    with pytest.raises(ValueError, match="event index outside"):
        assemble_replay_setups(bars=bars, events=[StreamEvent(2, None, None, None, None)], context=[_ctx(0)], htf_permission={}, horizon_bars=60)
